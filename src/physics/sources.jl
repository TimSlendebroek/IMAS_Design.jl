...
function fusion_source!(cs::IMAS.core_sources, cp::IMAS.core_profiles; DD_fusion::Bool=false)
    # to avoid double counting, delete fusion sources that were not generated by FUSE (which have a "→" in their name)
    n = length(cs.source)
    for (k, source) in enumerate(reverse(cs.source))
        if source.identifier.index == 6 && !contains(source.identifier.name, "→")
            deleteat!(cs.source, n - k + 1)
        end
    end
...
function radiation_source!(dd::IMAS.dd)
    ...
    deleteat!(dd.core_sources.source, :radiation)
