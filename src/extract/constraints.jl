...
function update_ConstraintFunctionsLibrary!()
    ...
    ConstraintFunction(:min_q95, "%", dd -> (abs(dd.equilibrium.time_slice[].global_quantities.q_95) - dd.requirements.q95) / dd.requirements.q95, >, 0.0)
    ConstraintFunction(:min_qmin, "%", dd -> (minimum(abs.(dd.equilibrium.time_slice[].profiles_1d.q)) - dd.requirements.qmin) / dd.requirements.qmin, >, 0.0)
    ...
    ConstraintFunction(:min_alpha_t, "%", dd -> (alpha_t(dd) - 0.55) / 0.55, >, 0.0)
    #! format: on
